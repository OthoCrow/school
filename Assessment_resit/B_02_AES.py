...
def to_state():
    ...
    return block_dicts    # Converts plaintext to hexidecimal bytes
    plaintext = validate_input("Enter plaintext: ", 100)
    utf8_bytes = plaintext.encode("utf-8")
    deci_bytes = list(utf8_bytes)
    # Adds padding so each block is 16 bytes
...
def validate_input(prompt, length=None):
    ...
    while True:
        ...
        if length != None and len(response) > length:
            print(f"Please enter {length} characters or less: ")
            continue
        return response
...
print(
        f"""This is a program that for encrypting text. Encrypting is conveting plain text into ciphertext (or a code), like spies do.
